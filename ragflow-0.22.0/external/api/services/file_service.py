...
from external.api.utils.file_utils import filename_type, read_potential_broken_pdf, thumbnail_img
from external.api.utils.abstract_extract_utils import summarize_input
...
class FileService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def upload_document(self, kb, file_objs, user_id, src="local", generate_summary=False):
        ...
        for index, file in enumerate(file_objs):
            try:
                ...
                # 不在指定文件类型范围里的文件用文件名生成缩略图
                # if img is None:
                #     if re.match(r".*\.(txt|md)$", filename):
                #         img = generate_text_thumbnail_bytes()
                #     img = _placeholder_thumbnail(filename)

                if img is not None:
                    thumbnail_location = f"thumbnail_{doc_id}.png"
                    settings.STORAGE_IMPL.put(kb.id, thumbnail_location, img)

                parse_text = self.parse(filename, blob, tenant_id=user_id)
                summary = ""
                if generate_summary:
                    # file.seek(0)
                    summary = summarize_input(text=parse_text, filename=filename, flow="generate")["summary"]
                ...
                doc = {
                    ...
                    "meta_fields": {"name": os.path.splitext(filename)[0], "tags": [], "summary": summary},
                }
    ...
    def parse(filename, blob, img_base64=True, tenant_id=None):
        ...
        kwargs = {"lang": "English", "callback": dummy, "parser_config": parser_config, "from_page": 0, "to_page": 100000, "tenant_id": tenant_id}
