...
class DocumentService(CommonService):
    ...
    def get_list(cls, kb_id, page_number, items_per_page, orderby, desc, keywords, ids, name, suffix=None, run=None):
        ...
        if ids:
            docs = docs.where(cls.model.id.in_(ids))
        if name:
    ...
    def get_unfinished_docs(cls):
        ...
        docs = cls.model.select(*fields).where(
            cls.model.status == StatusEnum.VALID.value, ~(cls.model.type == FileType.VIRTUAL.value), (((cls.model.progress < 1) & (cls.model.progress > 0)) | (cls.model.id.in_(unfinished_task_query)))
